...
tft.setLineWidth(4);
//tft.fillCircle(0,0,40,DISPLAY_BLACK);
tft.drawLine(gauge_needle_last_x *6/8,gauge_needle_last_y *6/8,gauge_needle_last_x*5/8,gauge_needle_last_y*5/8,DISPLAY_BLACK);
...
gauge_needle_last_x = cx;
gauge_needle_last_y = cy;
...
void loop(void)
{
  ...
  tft.setCursor(45,84);
  tft.setTextSize(2);
  float actual_value = sinVal * upper_limit;

  String stringOne =  String(lround(actual_value));                        // using a long and a base
  //
  tft.print(stringOne);
